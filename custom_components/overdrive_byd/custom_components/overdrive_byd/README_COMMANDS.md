...
v1.1.0 uses the current OverDrive vehicle-control protocol implemented by `VehicleControlCatalog` and `MqttCommandRouter`.
...
Enable **Allow vehicle control** for the MQTT connection in OverDrive. The app intentionally does not subscribe to control topics when this option is disabled.
